...
class InvoiceController extends Controller
{
    ...
    private function calculateItems(Invoice $invoice)
    {
        ...
        $student->loadMissing(['activeEnrollments.classModel', 'activeSportEnrollments.sport']);
        $activeEnrollments = $student->activeEnrollments;
        $dueDate = $this->makeDueDate($invoice->year, $invoice->month, (int) ($student->due_day ?? Setting::getPaymentDueDay()));
        ...
        $monthlyFees = MonthlyFee::with('classModel')
        ...
        // Add school material usages
        $materialUsages = SchoolMaterialUsage::where('student_id', $invoice->student_id)
            ->where(function($q) use ($invoice) {
                $q->whereNull('invoice_id')
                  ->orWhere('invoice_id', $invoice->id);
            })
            ->whereYear('usage_date', $invoice->year)
            ->whereMonth('usage_date', $invoice->month)
            ->get();
        ...
        foreach ($materialUsages as $usage) {
            ...
            // Link usage to this invoice if not already linked
            if (!$usage->invoice_id) {
    ...
    /**
     * Apply manual punctual discount (20%).
     */
    public function applyPunctualDiscount(Invoice $invoice)
    {
        if (!auth()->user()?->canManageFinancial()) {
            return back()->with('error', 'Você não tem permissão para aplicar descontos.');
        }

        if (in_array($invoice->status, ['paid', 'cancelled'])) {
            return back()->with('error', 'Não é possível alterar desconto em fatura paga ou cancelada.');
        }

        $discount = $invoice->punctual_discount_amount;

        if ($discount <= 0) {
            return back()->with('error', 'Não há valor para aplicar desconto nesta fatura.');
        }

        $invoice->discount = $discount;
        $invoice->total = max(0, (float) $invoice->subtotal - $discount);
        $invoice->save();

        return back()->with('success', 'Desconto manual de pontualidade (20%) aplicado.');
    }
    ...
    {
        if (!auth()->user()?->canManageFinancial()) {
            return back()->with('error', 'Você não tem permissão para remover descontos.');
        }
        ...
        return back()->with('success', 'Desconto removido da fatura.');
    }
    ...
    private function resolveMonthlyFeeAmount(Student $student, ?\App\Models\ClassModel $classModel = null): float
    {
