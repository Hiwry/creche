...
class AttendanceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $classes = ClassModel::active()
            ...
            ->with(['activeEnrollments' => function($q) use ($request) {
                ...
                })->with(['student.guardian', 'student.authorizedPersons']);
            }])
    ...
    public function quickRegister(Request $request)
    {
        ...
        if ($request->type === 'check_in') {
            $log->check_in = $now;
            $log->dropped_off_by = $request->responsible_name;
        } else {
            $log->check_out = $now;
            $log->picked_up_by = $request->responsible_name;
    ...
    public function update(Request $request, AttendanceLog $log)
    {
        $request->validate([
            ...
            'dropped_off_by' => 'nullable|string|max:255',
        ...
        $log->dropped_off_by = $request->dropped_off_by;
        $log->picked_up_by = $request->picked_up_by;
        $log->notes = $request->notes;
        $log->save();
