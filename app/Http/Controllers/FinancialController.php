...
class FinancialController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if (!in_array($sort, ['latest', 'alpha_asc', 'alpha_desc'], true)) {
            $sort = 'latest';
        }
        ...
        if (in_array($sort, ['alpha_asc', 'alpha_desc'], true)) {
            $direction = $sort === 'alpha_desc' ? 'desc' : 'asc';
     ...
     */
    public function reconcileFees(Student $student)
    {
        $reconciler = new \App\Services\Financial\FeeReconciler();
        $results = $reconciler->reconcileStudent($student);
        
        $message = "Sincronização concluída: {$results['fixed']} mensalidades corrigidas.";
        
        return back()->with('success', $message);
    }

    /**
     * Reconcile fees for ALL students.
     */
    public function bulkReconcileFees()
    {
        // Increase memory limit and execution time for bulk operation
        ini_set('memory_limit', '512M');
        set_time_limit(300);

        $reconciler = new \App\Services\Financial\FeeReconciler();
        $students = Student::whereHas('monthlyFees', function($q) {
        ...
        $message = "Sincronização em massa concluída! Verificados {$students->count()} alunos. {$totalFixed} mensalidades corrigidas.";
