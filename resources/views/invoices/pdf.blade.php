...
<body>
    ...
    <div class="rounded-box">
        ...
        <div class="company-details">
            ...
            CNPJ: {{ $settings['company_cnpj'] ?? $settings['company_document'] ?? '00.000.000/0000-00' }}<br>
    ...
    <div class="rounded-box" style="padding: 10px;">
        <div class="client-box-header">
            <strong>Aluno(a):</strong> {{ $invoice->student?->name ?? 'Aluno(a) Removido(a)' }}
        </div>
        <table class="client-table">
            <tr>
                <td style="background-color: #f4caa6;">Aluno(a):</td>
                <td style="background-color: #f4caa6;">{{ $invoice->student?->name ?? 'N/A' }}</td>
            </tr>
            <tr>
                <td>Responsável:</td>
            ...
            <tr>
                ...
                <td>{{ $invoice->student?->address ?? 'Endereço não cadastrado' }}</td>
            </tr>
            @if($invoice->student?->guardian && $invoice->student->guardian->address)
            <tr>
                <td>Endereço de cobrança:</td>
                <td>{{ $invoice->student->guardian->full_address }}</td>
            </tr>
            @endif
            <tr>
                <td>Local:</td>
        ...
        <table class="items-table">
            ...
            <tbody>
                ...
                <tr class="discount-row">
                    ...
                    <td>Desconto de pontualidade (manual)</td>
                    <td class="right">-</td>
    ...
    <div class="terms-text">
        Referência da fatura: <strong>{{ $invoice->reference }}</strong><br>
        Desconto de pontualidade (20%) se pago até o vencimento (aplicação manual): R$ {{ number_format($invoice->punctual_discount_amount, 2, ',', '.') }}<br>
    ...
    <div class="bottom-section">
        <div class="payment-box">
            ...
            <strong>Pix:</strong> {{ $settings['invoice_pix_key'] ?? $settings['pix_key'] ?? $settings['company_cnpj'] ?? $settings['company_document'] ?? 'CNPJ/CPF da Chave' }}<br>
            <strong>Banco:</strong> {{ $settings['invoice_bank_info'] ?? $settings['bank_name'] ?? 'Banco Exemplo' }}<br>
