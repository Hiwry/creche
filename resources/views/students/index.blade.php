...
<div class="action-bar">
    ...
    <div class="action-bar-right" style="display: flex; gap: 10px;">
        <form action="{{ route('financial.reconcile-all') }}" method="POST" onsubmit="return confirm('ATENÇÃO: Isso irá verificar TODOS os alunos com mensalidades pendentes e tentar corrigir inconsistências (como faturas pagas mas mensalidade pendente, ou faturas deletadas). Isso pode levar alguns segundos. Continuar?')">
            @csrf
            <button type="submit" class="btn btn-warning" title="Corrigir todas as pendências fantasmas">
                <i class="fas fa-sync-alt"></i> Reconciliar Tudo
            </button>
        </form>
                        ...
                        <div style="display: flex; align-items: center; gap: 12px;">
                            ...
                            <div>
                                ...
                                <div style="font-size: 0.75rem; color: #9CA3AF; margin-top: 2px;">
                                    <i class="fas fa-location-dot" style="font-size: 0.7rem;"></i> {{ Str::limit($student->address, 30) }}
                                </div>
                                @endif
                            </div>
                        </div>
