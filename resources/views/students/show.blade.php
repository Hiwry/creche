...
<div class="grid grid-2">
    ...
    <div class="card">
        ...
        <div style="display: flex; gap: 20px; margin-bottom: 20px;">
            ...
            <div>
                ...
                @endif
                @if($student->address)
                <p style="color: #4B5563; font-size: 0.9rem; margin-top: 8px;">
                    <i class="fas fa-location-dot" style="color: #6366F1; margin-right: 5px;"></i>
        ...
        @if($student->activeEnrollments->count() > 0 || $student->activeSportEnrollments->count() > 0)
        <div style="margin-bottom: 15px;">
            <span style="font-size: 0.85rem; color: #6B7280;">Turmas & Esportes:</span>
    ...
    <div class="card">
        ...
        <div style="margin-top: 15px; padding-top: 15px; border-top: 1px dashed #E5E7EB;">
            <span style="font-size: 0.85rem; color: #6B7280;">Endereço do Responsável:</span>
            <div style="font-weight: 500; font-size: 0.9rem;">{{ $student->guardian->full_address }}</div>
        </div>
        @endif
        @else
        <p style="color: #9CA3AF; padding: 15px;">Nenhum responsável cadastrado</p>
        @endif
        
        
        @if($student->authorizedPersons->count() > 0)
        <div style="margin-top: 20px; padding-top: 20px; border-top: 1px solid #E5E7EB;">
            <span style="font-size: 0.85rem; color: #6B7280; display: block; margin-bottom: 10px;">Pessoas Autorizadas a Buscar:</span>
            <div style="display: grid; gap: 10px;">
                @foreach($student->authorizedPersons as $person)
                <div style="display: flex; justify-content: space-between; align-items: center; background: #F3F4F6; padding: 10px; border-radius: 8px;">
                    <div>
                        ...
                        <div style="font-size: 0.8rem; color: #6B7280;">
                            @if($person->phone) Tel: {{ $person->phone }} @endif
                            @if($person->phone && $person->whatsapp) | @endif
                            @if($person->whatsapp) WhatsApp: {{ $person->whatsapp }} @endif
                        </div>
                    </div>
                </div>
                @endforeach
...
<div class="card" style="margin-top: 20px;">
    <div class="card-header">
        ...
        <div style="display: flex; gap: 10px;">
            <form action="{{ route('financial.reconcile-fees', $student) }}" method="POST" onsubmit="return confirm('Isso irá verificar mensalidades pendentes sem fatura ativa e marcá-las como pagas/canceladas. Continuar?')">
                @csrf
                <button type="submit" class="btn btn-warning btn-sm" title="Corrigir pendências fantasmas">
    ...
    <div class="card" style="margin-top: 20px;">
        <div class="card-header" style="display: flex; justify-content: space-between; align-items: center;">
            <h3 class="card-title">
                <i class="fas fa-running" style="color: #10B981; margin-right: 10px;"></i>
                Matrículas em Esportes
            </h3>
            <a href="{{ route('sports.index') }}" class="btn btn-sm btn-primary">
                <i class="fas fa-plus"></i> Gerenciar Esportes
            </a>
        </div>
        ...
        <div class="table-container">
            <table>
                ...
                <tbody>
                    @forelse($student->activeSportEnrollments as $enrollment)
                    <tr>
                        <td>
                            <div style="font-weight: 500;">{{ $enrollment->sport->name ?? '-' }}</div>
                        </td>
                        <td>R$ {{ number_format($enrollment->monthly_fee, 2, ',', '.') }}</td>
                        <td>{{ $enrollment->start_date ? $enrollment->start_date->format('d/m/Y') : '-' }}</td>
                        <td>
                            <form action="{{ route('sports.unenroll', $enrollment->id) }}" method="POST" onsubmit="return confirm('Encerrar esta matrícula?')">
                                @csrf
                    ...
                    <tr>
                        <td colspan="4" style="text-align: center; padding: 20px; color: #6B7280;">
                            Nenhuma matrícula em esportes encontrada.
                        </td>
                    </tr>
                    @endforelse
                </tbody>
