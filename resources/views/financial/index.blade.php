...
<div class="action-bar">
    ...
    <div class="action-bar-right">
        @if($canManageFinancial || auth()->user()->isTeacher())
...
<div class="card" style="margin-bottom: 20px;">
    <form action="{{ route('financial.index') }}" method="GET" class="filter-form">
        ...
        <select name="sort" class="form-control">
            ...
            <option value="alpha_asc" {{ ($sort ?? 'latest') === 'alpha_asc' ? 'selected' : '' }}>Aluno (A-Z)</option>
            <option value="alpha_desc" {{ ($sort ?? 'latest') === 'alpha_desc' ? 'selected' : '' }}>Aluno (Z-A)</option>
        </select>
